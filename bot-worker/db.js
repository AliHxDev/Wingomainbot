...
function normalizeConnectionString(value) {
  return value && !value.includes('sslmode=') ? `${value}${value.includes('?') ? '&' : '?'}sslmode=require` : value;
}
...
async function init() {
  ...
  pool = new Pool({ connectionString: normalizeConnectionString(process.env.DATABASE_URL), max: Number(process.env.DB_POOL_MAX || 4), connectionTimeoutMillis: 10_000 });
  await pool.query(`
